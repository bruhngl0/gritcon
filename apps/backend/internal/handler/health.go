...
import (
	...
	"github.com/bruhngl0/gritcon/internal/middleware"
...
type HealthHandler struct {
	Handler //handler struct is defined in base.go, since both the files are under the same package, we have directly used it and not imported.
}

func NewHealthHandler(s *server.Server) *HealthHandler {
	return &HealthHandler{
		Handler: NewHandler(s), //this is how we wrap a base handler in every other handler, since this health handle
		//is a system route, it is not necessary to wrap base handler in this as it is mainly defined for the DTO'S
	}
}
